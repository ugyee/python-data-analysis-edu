  ...
  'customer-clustering': {
    ...
    chapters: [
      ...
      },
      {
        ...
        title: '客户分群实战',
        lessons: [
          { id: 'l3', title: 'RFM模型', type: 'article', duration: 25, content: '# RFM模型\n\n## RFM定义\n\n- **R (Recency)**：最近一次购买时间\n- **F (Frequency)**：购买频率\n- **M (Monetary)**：消费金额\n\n## 计算RFM\n\n```python\nimport pandas as pd\n\n# 创建模拟交易数据\ndf = pd.DataFrame({\n    \'customer_id\': [1, 1, 2, 2, 3, 3, 3],\n    \'order_id\': [101, 102, 103, 104, 105, 106, 107],\n    \'date\': [\'2024-01-15\', \'2024-01-20\', \'2024-01-10\', \n              \'2024-01-25\', \'2024-01-05\', \'2024-01-12\', \'2024-01-30\'],\n    \'amount\': [100, 200, 150, 300, 50, 180, 250]\n})\n\ndf[\'date\'] = pd.to_datetime(df[\'date\'])\n\n# 计算RFM\nreference_date = df[\'date\'].max() + pd.Timedelta(days=1)\n\nrfm = df.groupby(\'customer_id\').agg({\n    \'date\': lambda x: (reference_date - x.max()).days,  # Recency\n    \'order_id\': \'count\',  # Frequency\n    \'amount\': \'sum\'  # Monetary\n})\n\nrfm.columns = [\'R\', \'F\', \'M\']\nprint(rfm)\n```' },
          { id: 'l4', title: '确定最佳聚类数', type: 'article', duration: 25, content: '# 确定最佳聚类数\n\n## 肘部法则\n\n```python\nimport matplotlib.pyplot as plt\nimport numpy as np\nimport pandas as pd\n\n# 创建模拟数据\nnp.random.seed(42)\nrfm = pd.DataFrame({\n    \'R\': np.random.randint(1, 100, 100),\n    \'F\': np.random.randint(1, 20, 100),\n    \'M\': np.random.randint(100, 5000, 100)\n})\n\n# 标准化\nrfm_scaled = rfm.apply(lambda x: (x - x.mean()) / x.std())\n\n# K-Means实现\ndef kmeans_simple(data, k, max_iter=100):\n    data_values = data.values\n    np.random.seed(42)\n    centers = data_values[np.random.choice(len(data_values), k, replace=False)]\n    for _ in range(max_iter):\n        distances = np.sqrt(((data_values[:, np.newaxis] - centers)**2).sum(axis=2))\n        labels = distances.argmin(axis=1)\n        new_centers = np.array([data_values[labels == i].mean(axis=0) for i in range(k)])\n        if np.allclose(centers, new_centers):\n            break\n        centers = new_centers\n    # 计算SSE\n    sse = sum(np.min(np.sqrt(((data_values - c)**2).sum(axis=1)))**2 for c in centers)\n    return sse\n\n# 计算不同K值的SSE\nsse = []\nK_range = range(1, 11)\n\nfor k in K_range:\n    sse.append(kmeans_simple(rfm_scaled, k))\n\n# 绘制肘部图\nplt.plot(K_range, sse, \'bx-\')\nplt.xlabel(\'k\')\nplt.ylabel(\'SSE\')\nplt.title(\'Elbow Method\')\nplt.show()\n```' },
          { id: 'l5', title: '客户分群实现', type: 'article', duration: 30, content: '# 客户分群实现\n\n```python\nimport pandas as pd\nimport numpy as np\nimport matplotlib.pyplot as plt\n\n# 1. 创建模拟RFM数据\nnp.random.seed(42)\nrfm = pd.DataFrame({\n    \'customer_id\': range(1, 101),\n    \'R\': np.random.randint(1, 100, 100),\n    \'F\': np.random.randint(1, 20, 100),\n    \'M\': np.random.randint(100, 5000, 100)\n})\n\n# 2. 数据标准化（手动实现）\nrfm_scaled = rfm[[\'R\', \'F\', \'M\']].apply(lambda x: (x - x.mean()) / x.std())\n\n# 3. 简单K-Means实现（简化版）\ndef kmeans_simple(data, k, max_iter=100):\n    data_values = data.values\n    np.random.seed(42)\n    centers = data_values[np.random.choice(len(data_values), k, replace=False)]\n    for _ in range(max_iter):\n        distances = np.sqrt(((data_values[:, np.newaxis] - centers)**2).sum(axis=2))\n        labels = distances.argmin(axis=1)\n        new_centers = np.array([data_values[labels == i].mean(axis=0) for i in range(k)])\n        if np.allclose(centers, new_centers):\n            break\n        centers = new_centers\n    return labels, centers\n\nlabels, centers = kmeans_simple(rfm_scaled, 4)\nrfm[\'Cluster\'] = labels\n\n# 4. 分析各群体特征\ncluster_stats = rfm.groupby(\'Cluster\').agg({\n    \'R\': \'mean\',\n    \'F\': \'mean\',\n    \'M\': \'mean\',\n    \'customer_id\': \'count\'\n}).rename(columns={\'customer_id\': \'Count\'})\n\nprint(cluster_stats)\n\n# 5. 可视化\nplt.scatter(rfm[\'R\'], rfm[\'F\'], c=rfm[\'Cluster\'], cmap=\'viridis\')\nplt.xlabel(\'Recency\')\nplt.ylabel(\'Frequency\')\nplt.title(\'Customer Segments\')\nplt.show()\n```' },
        ],
      },
      {
        id: 'ch3',
        title: '章节测验',
        lessons: [
          { id: 'l6', title: '章节测验', type: 'quiz', duration: 10, content: '' },
        ],
      },
    ],
  },
